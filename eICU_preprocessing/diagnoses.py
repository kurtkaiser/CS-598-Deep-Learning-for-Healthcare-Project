...
def add_codes(splits, codes_dict, words_dict, count):
    ...
    if levels >= 1:
        s0 = splits[0]
        try:
            codes.append(codes_dict[s0][0])
            codes_dict[s0][2] += 1
        except KeyError:
            codes_dict[s0] = [count, {}, 0]
            codes.append(count)
            words_dict[count] = s0
            count += 1
    if levels >= 2:
        ...
        s1 = splits[1]
        try:
            codes.append(codes_dict[s0][1][s1][0])
            codes_dict[s0][1][s1][2] += 1
        except KeyError:
            codes_dict[s0][1][s1] = [count, {}, 0]
            codes.append(count)
            words_dict[count] = s0 + '|' + s1
            count += 1
    if levels >= 3:
        s0 = splits[0]
        s1 = splits[1]
        try:
            codes.append(codes_dict[s0][1][s1][1][splits[2]][0])
            codes_dict[s0][1][s1][1][splits[2]][2] += 1
        except KeyError:
            codes_dict[s0][1][s1][1][splits[2]] = [count, {}, 0]
            codes.append(count)
            words_dict[count] = s0 + '|' + s1 + '|' + splits[2]
            count += 1
    if levels >= 4:
        s0 = splits[0]
        s1 = splits[1]
        try:
            codes.append(codes_dict[s0][1][s1][1][splits[2]][1][splits[3]][0])
            codes_dict[s0][1][s1][1][splits[2]][1][splits[3]][2] += 1
        except KeyError:
            codes_dict[s0][1][s1][1][splits[2]][1][splits[3]] = [count, {}, 0]
            codes.append(count)
            words_dict[count] = s0 + '|' + s1 + '|' + splits[2] + '|' + splits[3]
            count += 1
    if levels >= 5:
        s0 = splits[0]
        s1 = splits[1]
        try:
            codes.append(codes_dict[s0][1][s1][1][splits[2]][1][splits[3]][1][splits[4]][0])
            codes_dict[s0][1][s1][1][splits[2]][1][splits[3]][1][splits[4]][2] += 1
        except KeyError:
            # print('key error')
            codes_dict[s0][1][s1][1][splits[2]][1][splits[3]][1][splits[4]] = [count, {}, 0]
            codes.append(count)
            words_dict[count] = s0 + '|' + s1 + '|' + splits[2] + '|' + splits[3] + '|' + splits[4]
            count += 1
    if levels is 6:
        s0 = splits[0]
        s1 = splits[1]
        try:
            codes.append(codes_dict[s0][1][s1][1][splits[2]][1][splits[3]][1][splits[4]][1][splits[5]][0])
            codes_dict[s0][1][s1][1][splits[2]][1][splits[3]][1][splits[4]][1][splits[5]][2] += 1
        except KeyError:
            codes_dict[s0][1][s1][1][splits[2]][1][splits[3]][1][splits[4]][1][splits[5]] = [count, {}, 0]
            codes.append(count)
            words_dict[count] = s0 + '|' + s1 + '|' + splits[2] + '|' + splits[3] + '|' + splits[4] + '|' + splits[5]
            count += 1
    #print(codes)
    return codes, count
...
def get_mapping_dict(unique_diagnoses):
    ...
    for diagnosis in adm_diagnoses:
        ...
        shortened = diagnosis.replace('admission diagnosis|', '').replace('All Diagnosis|', '')
...
def add_apache_diag(sparse_df, eICU_path, cut_off):
    ...
    # print('efficient direct return testing')
    return sparse_df.join(adm_diag, how='outer', on='patientunitstayid')
