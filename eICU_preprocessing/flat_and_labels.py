...
def preprocess_flat(flat):
    ...
    feature_names = ['ethnicity', 'unittype', 'unitadmitsource', 'unitvisitnumber', 'unitstaytype','physicianspeciality', 'numbedscategory', 'region']
    ...
    flat = pd.get_dummies(flat, columns=feature_names)
    ...
    min_max_feat = ['admissionweight', 'age', 'hour']

    def scale_min_max(flat):
        q = flat.quantile([0.05, 0.95])
        mins = q.loc[0.05]
        return 2 * (flat - mins) / (q.loc[0.95] - mins) - 1

    flat[min_max_feat] = flat[min_max_feat].apply(scale_min_max)
    ...
    flat[min_max_feat] = flat[min_max_feat].clip(lower=-4, upper=4)
    ...
    weight_str = 'admissionweight'
    height_str = 'admissionheight'
    flat['nullweight'] = flat[weight_str].isnull().astype(int)
    flat['nullheight'] = flat[height_str].isnull().astype(int)
    flat[weight_str].fillna(0, inplace=True)
    flat[height_str].fillna(0, inplace=True)
