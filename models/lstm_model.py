...
class BaseLSTM(nn.Module):
    def __init__(self, config, F=None, D=None, no_flat_features=None):
        ...
        low_str = 'low_momentum'
        self.momentum = 0.01 if self.batchnorm == low_str else 0.1
        ...
        if self.batchnorm in ['mybatchnorm', low_str]:
        ...
        input_size = self.n_units * channel_wise
        # print(input_size)
        input_size = input_size + self.diagnosis_size + self.no_flat_features
        # print(input_size)
        ...
        if self.batchnorm in ['mybatchnorm', 'pointonly', low_str]:
    ...
    def init_hidden(self, B, device):
        return (torch.zeros(self.n_layers * self.n_dir, B, self.n_units).to(device), torch.zeros(self.n_layers * self.n_dir, B, self.n_units).to(device))
    ...
    def forward(self, X, diagnoses, flat, time_before_pred=5):
        ...
        if self.channelwise is False:
            ...
            split_x = torch.split(X[:, 1:-1, :], self.F, dim=1)  # tuple ((B * F * T), (B * F * T))
            stack_x = torch.stack(split_x, dim=2)  # B * F * 2 * T
            lstm_output = None
            for i in range(self.F):
                X_lstm, hidden = self.channelwise_lstm_list[i](stack_x[:, i, :, :].permute(2, 0, 1))
                lstm_output = cat(self.remove_none((lstm_output, X_lstm)), dim=2)
        # print(lstm_output)
        relu_x = self.relu(self.lstm_dropout(lstm_output.permute(1, 2, 0)))
        ...
        if self.no_diag:
            combined_features = cat((flat.repeat_interleave(T - time_before_pred, dim=0),  # (B * (T - time_before_pred)) * no_flat_features
                                     relu_x[:, :, time_before_pred:].permute(0, 2, 1).contiguous().view(B * (T - time_before_pred), -1)), dim=1)
        else:
    ...
    def loss(self, y_hat_los, y_hat_mort, y_los, y_mort, mask, seq_lengths, device, sum_losses, loss_type):
        ...
        # los loss
            ...
            if loss_type == 'msle':
                los_loss = self.msle_loss(y_hat_los, y_los, mask.type(torch.cuda.BoolTensor if device == torch.device('cuda') else torch.BoolTensor), seq_lengths, sum_losses)
            elif loss_type == 'mse':
                los_loss = self.mse_loss(y_hat_los, y_los, mask.type(torch.cuda.BoolTensor if device == torch.device('cuda') else torch.BoolTensor), seq_lengths, sum_losses)
            if self.task == 'LoS':
                loss = los_loss
