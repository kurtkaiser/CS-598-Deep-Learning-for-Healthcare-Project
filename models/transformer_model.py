...
class TransformerEncoder(nn.Module):
    def __init__(self, input_size=None, d_model=None, num_layers=None, num_heads=None, feedforward_size=None, dropout=None, pe=None, device=None):
...
class Transformer(nn.Module):
    ...
    def forward(self, X, diagnoses, flat, time_before_pred=5):
        ...
        relu_x = self.relu(self.trans_dropout(self.transformer(X, T)))  # B * d_model * T
        ...
        if not self.no_diag:
            ...
            combined_features = cat((flat.repeat_interleave(T - time_before_pred, dim=0),
                                     relu_x[:, :, time_before_pred:].permute(0, 2, 1).contiguous().view(B * (T - time_before_pred), -1)), dim=1)
        ...
        return los_predictions, self.sigmoid(self.point_final_mort(last_point_mort).view(B, T - time_before_pred))
