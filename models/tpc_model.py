...
class MSLELoss(nn.Module):
    ...
    def forward(self, y_hat, y, mask, seq_length, sum_losses=False):
        ...
        loss = self.squared_error(log_y_hat, y.log().where(mask, torch.zeros_like(y)))
...
class MSELoss(nn.Module):
    ...
    def forward(self, y_hat, y, mask, seq_length, sum_losses=False):
        ...
        # where there is no data log_y_hat = log_y = 0, so the squared error will be 0 in these places
        loss = self.squared_error(y_hat, y.where(mask, torch.zeros_like(y)))
        loss = torch.sum(loss, dim=1)
        if not sum_losses:
            c = seq_length.clamp(min=1)
            loss = loss / c
        return loss.mean()
    ...
    def forward(self, input):
        ...
        if self.track_running_stats:
            if self.num_batches_tracked is not None:
                ...
                if self.momentum is not None:  # exp average
                    exponential_average_factor = self.momentum if self.training else self.eval_momentum
                else:  # moving avg
                    exponential_average_factor = 1.0 / float(self.num_batches_tracked)

        batch_f =  F.batch_norm(input, self.running_mean, self.running_var, self.weight, self.bias, training=True, momentum=exponential_average_factor, eps=self.eps)
        return batch_f # set training to True so it calculates the norm of the batch
...
class TempPointConv(nn.Module):
    ...
    def init_temp(self):
        ...
        y1 = 1 + self.Y
        # print(y1)
        input_size = self.F * y1 + self.diagnosis_size + self.no_flat_features
        res = nn.Linear(in_features=input_size, out_features=self.last_linear_size)
        self.point_last_los = res
        self.point_last_mort = res
        return
    ...
    def create_temp_pointwise_layers(self):
        ...
        for i in range(self.n_layers):
            ...
            temp = nn.Conv1d(in_channels=temp_in_channels,  # (F + Zt) * (Y + 1)
                             ...
                             stride=self.layers[i][stride_str],
                             dilation=self.layers[i][dil_str],
            ...
            if self.no_skip_connections:
                ...
                temp = nn.Conv1d(in_channels=temp_in_channels, out_channels=temp_out_channels, kernel_size=self.kernel_size, stride=self.layers[i][stride_str], dilation=self.layers[i][dil_str],groups=self.F)
    ...
    def create_temp_only_layers(self):
        ...
        self.Y = 0
        self.layer_modules = nn.ModuleDict()
        temp_str = 'temp_kernels'

        for i in range(self.n_layers):
            #print('flipped for speed')
            if not self.share_weights:
                temp_in_channels = self.F * (1 + self.Y) if i > 0 else 2 * self.F  # F * (Y + 1)
                temp_out_channels = self.F * self.layers[i][temp_str]  # F * temp_kernels
                groups = self.F
            else:
                temp_in_channels = (1 + self.Y) if i > 0 else 2  # (Y + 1)
                temp_out_channels = self.layers[i][temp_str]
                groups = 1

            temp = nn.Conv1d(in_channels=temp_in_channels, out_channels=temp_out_channels,kernel_size=self.kernel_size,stride=self.layers[i]['stride'],dilation=self.layers[i]['dilation'],groups=groups)
